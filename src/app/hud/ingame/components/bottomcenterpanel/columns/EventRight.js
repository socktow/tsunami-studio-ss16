...
const EventRight = ({
  ...
  hasBaron,
  hasElder,
  respawnAt // Nhận thời gian hồi sinh từ props
}) => {
  const [isLevelingUp, setIsLevelingUp] = useState(false);
  const prevLevelRef = useRef(level);

  // Xác định trạng thái chết và làm tròn thời gian đếm ngược
  ...
  return (
    <div className="relative w-[45px] h-[45px]">
      ...
      <div className={`relative w-full h-full overflow-hidden rounded bg-zinc-900 border border-white/10 transition-all duration-500 ${isDead ? "border-zinc-700" : ""}`}>
        ...
        {/* 2. CHAMPION IMAGE - Mờ và xám khi chết */}
        <img
        ...
        {/* 3. RESPAWN TIMER - Số đếm giây đỏ với hiệu ứng Pulse */}
        <AnimatePresence>
          {isDead && (
            <motion.div
        ...
        {/* 5. LEVEL BADGE (Ẩn khi chết để tập trung vào số đếm) */}
      ...
      {/* 7. ULTIMATE ICON (Nằm bên trái cho phía Right side) */}
      {ulti && (
        <img
          src={`${IMAGE_BASE_URL}${ulti}`}
          className={`absolute top-0.5 -left-4.5 w-[25px] h-[25px] rounded-full border border-black shadow-[0_0_8px_rgba(34,197,94,0.9)] bg-black z-50 transition-all duration-500 ${isDead ? "grayscale opacity-40 scale-75" : "grayscale-0 opacity-100 scale-100"}`}
          alt="ultimate"
        />
      )}
    </div>
