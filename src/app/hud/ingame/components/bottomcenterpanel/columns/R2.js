...
// --- DANH MỤC NGỌC (Nên đưa vào file constants nếu dùng chung) ---
...
const R2 = () => {
  ...
  const renderPlayerContent = (i) => {
    ...
    // Lấy thông tin ngọc phụ (Vị trí index 4 thường là ngọc phụ đầu tiên)
    ...
    return (
      <div className={`flex items-center w-full h-full px-1 justify-between transition-all duration-500 ${isDead ? "grayscale opacity-60" : "opacity-100"}`}>
        ...
        {/* ITEMS GRID (Bên phải cho Red Team) */}
        <motion.div variants={containerVariants} initial="hidden" animate="visible" className="flex items-center gap-[3px]">
              ...
              {slot.item ? (
                <>
                  ...
                  {(slot.item.stacks > 0 || slot.item.charges > 0) && (
                    <div 
                      className="absolute bottom-0 left-0 min-w-[10px] h-[10px] px-[1px] flex items-center justify-center text-[9px] leading-none font-black z-40" 
