...
// 1. Swap motion out for LazyMotion, m, and domAnimation
...
const MatchHistory = () => {
  ...
  return (
    // 2. Wrap your component container with LazyMotion and pass features
          ...
          <header className="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-emerald-500/10 pb-10">
            <div>
              <div className="flex items-center gap-3 mb-4">
                ...
                <span className="text-[10px] font-semibold uppercase tracking-[0.5em] text-emerald-500/40 font-sans">Operation_Archive</span>
              </div>
              <h1 className="text-5xl font-semibold text-white uppercase italic tracking-tighter leading-none">
            ...
            <div className="flex flex-col gap-2">
               ...
               <div className="flex justify-between items-center px-2">
                  <span className="text-[8px] text-emerald-500/20 uppercase font-semibold tracking-widest leading-none">Live_Sync: Active</span>
                  <span className="text-[8px] text-emerald-500/20 uppercase font-semibold tracking-widest leading-none">Nodes: {MOCK_MATCHES.length}</span>
               </div>
            </div>
          </header>
          ...
          <div className="space-y-4">
            <AnimatePresence>
              {MOCK_MATCHES.map((match, idx) => (
                // 3. Changed from <motion.div> to <m.div>
                <m.div
                  ...
                  <div className="flex flex-col lg:flex-row items-center p-5 gap-6">
                    ...
                    <div className="flex flex-col w-full lg:w-56 border-r border-emerald-500/10 pr-6">
                      <div className="flex items-center gap-2 mb-2">
                         ...
                         <span className="text-[12px] text-white font-semibold tracking-widest group-hover:text-emerald-400 transition-colors">{match.game_id}</span>
                      </div>
                    ...
                    <div className="flex-1 flex items-center justify-between gap-2 w-full px-4">
                      ...
                      <div className="flex items-center gap-6 flex-1 justify-end">
                        <div className="text-right flex flex-col">
                          <span className={`text-2xl font-semibold italic tracking-tighter ${match.teamA.result === 'WIN' ? 'text-white' : 'text-zinc-700'}`}>
                             {match.teamA.name}
                          </span>
                          <span className={`text-[9px] font-semibold uppercase tracking-widest ${match.teamA.result === 'WIN' ? 'text-emerald-500' : 'text-red-900'}`}>
                      ...
                      <div className="flex flex-col items-center justify-center min-w-[80px]">
                         <div className="text-3xl font-semibold italic text-white flex items-center gap-1">
                      ...
                      <div className="flex items-center gap-6 flex-1 justify-start">
                        ...
                        <div className="text-left flex flex-col">
                          <span className={`text-2xl font-semibold italic tracking-tighter ${match.teamB.result === 'WIN' ? 'text-white' : 'text-zinc-700'}`}>
                             {match.teamB.name}
                          </span>
                          <span className={`text-[9px] font-semibold uppercase tracking-widest ${match.teamB.result === 'WIN' ? 'text-emerald-500' : 'text-red-900'}`}>
                    ...
                    <div className="flex items-center gap-4">
                      <button className="flex flex-col items-center gap-1 group/btn p-3 border border-emerald-500/10 hover:border-emerald-500/50 hover:bg-emerald-500/10 transition-all">
                         ...
                         <span className="text-[7px] font-semibold text-emerald-500/20 group-hover/btn:text-emerald-500">DATA_RECAP</span>
                      </button>
          ...
          <footer className="mt-16 py-8 border-t border-emerald-500/5 flex justify-between items-center">
             <div className="flex gap-10">
                <div className="flex flex-col gap-1">
                   <span className="text-[8px] text-zinc-600 uppercase font-semibold">Central_Server</span>
                   <span className="text-[10px] text-emerald-500/40 font-bold">STABLE // DA NANG NODE</span>
                </div>
                <div className="flex flex-col gap-1">
                   <span className="text-[8px] text-zinc-600 uppercase font-semibold">Last_Sync</span>
                   <span className="text-[10px] text-emerald-500/40 font-bold italic">2026-05-04 05:07:07</span>
                </div>
             </div>
             <div className="text-[10px] font-semibold text-emerald-500/10 tracking-[0.5em] uppercase">
